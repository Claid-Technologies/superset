...
```bash
superset workspaces list --local --project my-app --quiet | while read ws; do
  superset terminals list --local --workspace "$ws" --json 2>/dev/null \
    | jq -r '.sessions[].terminalId' | while read t; do
      echo "=== $ws / $t ==="
      superset terminals read --local --workspace "$ws" --terminal "$t" --max-lines 5 --json | jq -r '.text'
  done
done
```
...
superset workspaces list --local --project my-app --quiet | while read ws; do
  superset terminals list --local --workspace "$ws" --json 2>/dev/null \
    | jq -r '.sessions[].terminalId' | while read t; do
      superset terminals send --local --workspace "$ws" --terminal "$t" \
...
```bash
superset workspaces list --local --project my-app --quiet | xargs -L1 superset workspaces delete --local
```
