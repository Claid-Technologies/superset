...
<Command
	...
	options={[
		...
		{ flag: "--host <id>", description: "Create on this host (machineId) instead of in the cloud. Mutually exclusive with `--local`." },
		{ flag: "--local", description: "Create on this machine instead of in the cloud. Mutually exclusive with `--host`." },
...
<Command
	...
	options={[
		{ flag: "--host <id>", description: "List workspaces on this host (machineId) instead of the cloud." },
		{ flag: "--local", description: "List workspaces on this machine instead of the cloud." },
  ...
  tags: string;   // comma-joined tag set
	...
	quiet="workspace IDs"
>
List the organization's cloud workspaces, with their `status` (`provisioning`,
`ready`, `failed`). Pass `--local` for this machine's workspaces or `--host <id>`
for another host's; host listings have the columns shown above, and there is no
...
>
Create a workspace. Without `--local` or `--host` it is a **cloud workspace**, started
from `--environment` (or the first environment with repositories); the host-only flags
...
<Command
	...
	options={[
		{ flag: "--host <id>", description: "Host the workspaces live on (default: the cloud)." },
		{ flag: "--local", description: "The workspaces are on this machine." },
	]}
	output={`{ deleted: string[]; warnings: string[] }`}
>
Delete one or more cloud workspaces, tearing down their sandboxes (which is what
stops them billing), or workspaces on this machine with `--local` or another
host with `--host <id>`.
...
<Command
	...
	options={[
		{ flag: "--host <id>", description: "Host the workspace lives on (default: the cloud)." },
...
>
Show details for a single workspace: a cloud workspace by default, or one on
this machine (`--local`) or another host (`--host <id>`). `--field` is handy in
...
<Command
	...
	output="Workspace"
>
Rename a cloud workspace, or update a workspace on this machine (`--local`) or
another host (`--host <id>`). Tasks and tags exist only on host workspaces.
