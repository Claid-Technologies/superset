...
<Command
	...
	options={[
		{ flag: "--project <projectId>", description: "Project ID. Required unless --session or --cloud is set." },
		{ flag: "--session", description: "Create a project-less session (a managed scratch folder). Mutually exclusive with --project and --cloud." },
		{ flag: "--name <name>", description: "Workspace name. Required when `--project` is set; optional for sessions." },
		{ flag: "--checkout <local|worktree>", description: "Use the shared project checkout or an isolated worktree (default). Requires --project; incompatible with --cloud." },
