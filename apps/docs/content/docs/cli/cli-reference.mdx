...
<Command
	...
	options={[
		...
		{ flag: "--environment <id|name>", description: "Environment a cloud workspace starts from; its repositories are the checkouts. Defaults to the first environment with repositories." },
		{ flag: "--host <id>", description: "Create on this host (machineId). Mutually exclusive with `--local`." },
		{ flag: "--local", description: "Create on this machine. Mutually exclusive with `--host`." },
...
Either way, the project is registered on the host with no workspaces yet;
create one with `superset ws create --project <id> --name local --checkout local --local` (the
repo's checkout as it is) or `--branch <name>` (its own worktree).
...
<Command
	...
	options={[
		{ flag: "--host <id>", description: "List workspaces on this host (machineId)." },
		{ flag: "--local", description: "List workspaces on this machine." },
		{ flag: "--project <nameOrId>", description: "Filter by project name (case-insensitive) or id. Host workspaces only." },
		{ flag: "--search <text>", description: "Substring match against workspace name or branch. Alias: -s." },
		{ flag: "--tag <tag>", description: "Filter to workspaces carrying this tag (case-insensitive). Host workspaces only." },
	]}
	output={`Array<{
  id: string;
  name: string;
  ...
  tags: string;   // comma-joined tag set
	...
	quiet="workspace IDs"
>
List workspaces. If your account has cloud workspaces, the default is the
organization's cloud workspaces with their `status` (`provisioning`, `ready`,
`failed`); otherwise it is this machine's workspaces, as before. Pass `--local`
for this machine's workspaces or `--host <id>` for another host's; host listings have the columns shown above, and there is no
org-wide listing across hosts (the desktop app is the cross-host view).
`projectName` falls back to the raw project id when the host doesn't know the
project's name.
</Command>
...
<Command
	...
	options={[
		{ flag: "--project <projectId>", description: "Project ID, for a workspace on a host. Required with `--local` or `--host` unless `--session` is set." },
		{ flag: "--session", description: "Create a project-less session (a managed scratch folder) on a host. Mutually exclusive with `--project`." },
		{ flag: "--name <name>", description: "Workspace name. Required when `--project` is set; optional for sessions." },
		{ flag: "--checkout <local|worktree>", description: "Use the shared project checkout or an isolated worktree (default). Requires `--project`." },
		{ flag: "--branch <branch>", description: "Workspace branch. Required unless `--pr`, `--task`, or `--checkout local` is set." },
...
>
Create a workspace. Without `--local` or `--host`, an account with cloud workspaces
creates a **cloud workspace** (an account without them creates on this machine, as
before), started from `--environment` (or the first environment with repositories); the host-only flags
(`--project`, `--session`, `--checkout`, `--pr`, `--task`, `--base-branch`,
`--skip-branch-prefix`, `--tag`, `--attachment`, `--command`) are rejected there with a
hint to pass `--local` or `--host`. On a host, with `--project`, specify one of
`--checkout local`, `--branch`, `--pr`, or `--task` (`--branch` and `--pr` are mutually
exclusive). If you pass `--agent`, you must also pass `--prompt`. `--command`
runs a one-off shell command in the workspace directory and is independent of
`--agent`/`--prompt`. Pass either or both. `--model` and `--effort` require
`--agent`; see the [agent effort levels](#agent-effort-levels) below.

`--checkout local` shares the project's existing files, git index and checked-out branch.
It does not switch branches or run setup. Do not combine it with `--branch`, `--pr`,
`--base-branch`, or `--skip-branch-prefix`. Local creation requires a host that
supports local workspaces; older hosts reject it without creating a worktree.
...
`--base-branch`, `--checkout`, `--skip-branch-prefix`, and `--tag` are rejected, `--name` is
optional, and the output has no `alreadyExists` field. Host workspace creation
requires either `--project <id>` or `--session`; omitting both is an error.

```bash
# Cloud workspace from an environment, with an agent
superset workspaces create --environment Superset --agent claude --prompt "…"
...
<Command
	...
	options={[
		{ flag: "--host <id>", description: "Host the workspaces live on (default: the cloud if your account has cloud workspaces, else this machine)." },
		{ flag: "--local", description: "The workspaces are on this machine." },
	]}
	output={`{ deleted: string[]; warnings: string[] }`}
>
Delete one or more workspaces: cloud workspaces by default if your account has
them, tearing down their sandboxes (which is what stops them billing), otherwise
workspaces on this machine. `--local` or `--host <id>` picks a host.

```bash
superset workspaces delete ws_a ws_b ws_c
superset workspaces delete ws_d --local
```
</Command>
...
<Command
	...
	options={[
		{ flag: "--host <id>", description: "Host the workspace lives on (default: the cloud if your account has cloud workspaces, else this machine)." },
		{ flag: "--local", description: "The workspace is on this machine. Mutually exclusive with `--host`." },
  ...
  name: string;
  branch: string;
  type: "local" | "worktree" | "session" | "main"; // "main" on older hosts
  projectId: string;
  projectName: string;
  hostId: string;
  hostName: string;
...
>
Show details for a single workspace: a cloud workspace by default if your account
has them (otherwise one on this machine), or one on this machine (`--local`) or
another host (`--host <id>`). `--field` is handy in
scripts: `superset workspaces get --field worktreePath`.

```bash
superset workspaces get ws_…
superset workspaces get --local --field branch   # inside a local workspace
```
</Command>
...
<Command
	...
	output="Workspace"
>
Rename a cloud workspace (the default if your account has them), or update a
workspace on this machine (`--local`, and the default otherwise) or another host
(`--host <id>`). Tasks and tags exist only on host workspaces.
`--tag` replaces the whole tag set — tagging is how workspaces group into sidebar folders, so
retagging moves the workspace between folders.

```bash
superset workspaces update ws_… --name "better-name"
superset workspaces update ws_… --local --task-id tsk_…
superset workspaces update ws_… --local --tag perf --tag infra
superset workspaces update ws_… --local --clear-task
```
</Command>
...
<Command
	...
	options={[
		...
		{ flag: "--host <hostId>", description: "Host the automation runs on. Automations run on a host: pass this or `--local`." },
		{ flag: "--local", description: "Run the automation on this machine." },
...
<Command
	...
	options={[
		...
		{ flag: "--local", description: "Retarget to this machine." },
