...
superset workspaces list --local --quiet | xargs -L1 superset workspaces delete --local
```
...
Workspace commands (`workspaces`, `terminals`, `agents create`) default to cloud
workspaces if your account has them, and to this machine otherwise. `--local`
picks this machine and `--host <id>` another host either way. Commands that only make sense on a machine (`projects`, `agents
list`, `automations`) take the same flags. Local-targeted calls go
straight to the host server over loopback (offline-friendly); remote
calls route through the cloud API and the relay.

```bash
superset hosts list
superset workspaces list              # cloud workspaces if your account has them, else this machine
superset workspaces list --local      # this machine
