...
Workspace commands (`workspaces`, `terminals`, `agents create`) act on cloud
workspaces unless you pass `--local` for this machine or `--host <id>` for
another host. Commands that only make sense on a machine (`projects`, `agents
...
```bash
superset hosts list
superset workspaces list              # cloud workspaces
