...
An agent in any workspace can find every page you can read, including pages published from other workspaces or in earlier sessions. Page slugs end in a random suffix, so ask the agent to look the page up by name rather than guessing its URL:

```bash
superset pages list --search onboarding
superset pages pull <page-id-or-slug> > onboarding-design.html
```

This makes a page a durable place to keep notes, findings, and decisions that a later agent session should read back.
