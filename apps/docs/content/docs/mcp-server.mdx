...
| Tool | Description |
|------|-------------|
| `workspaces_list` | List cloud workspaces, or a host's workspaces with `hostId` (from `hosts_list`) |
| `workspaces_create` | Create a cloud workspace from an environment, or a workspace on a host with `hostId`, optionally launching agents |
| `workspaces_update` | Rename a cloud workspace, or update a host workspace's fields |
| `workspaces_delete` | Delete a cloud workspace (tears down its sandbox), or a host workspace with `hostId` |
...
Workspace tools act on cloud workspaces when you omit `hostId` and your account has cloud workspaces; otherwise `hostId` is required. The terminal tools and `agents_create` follow the same rule.
