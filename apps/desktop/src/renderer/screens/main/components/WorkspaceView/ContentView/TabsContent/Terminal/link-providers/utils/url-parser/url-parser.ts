...
let _classifier: Map<number, CharacterClass> | null = null;
function getClassifier(): Map<number, CharacterClass> {
	if (_classifier === null) {
		_classifier = new Map<number, CharacterClass>();
...
function createLink(
	classifier: Map<number, CharacterClass>,
	line: string,
	lineNumber: number,
	...
	do {
		...
		const chClass = classifier.get(chCode) ?? CharacterClass.None;
...
export function computeLinks(
	...
	for (let i = 1, lineCount = model.getLineCount(); i <= lineCount; i++) {
			...
			if (state === State.Accept) {
				...
				switch (chCode) {
					...
					case CharCode.Space:
						...
						break;
					default:
						chClass = classifier.get(chCode) ?? CharacterClass.None;
				}

				// Check if character terminates link
				if (chClass === CharacterClass.ForceTermination) {
					result.push(createLink(classifier, line, i, linkBeginIndex, j));
		...
		if (state === State.Accept) {
			result.push(createLink(classifier, line, i, linkBeginIndex, len));
		}
	}
