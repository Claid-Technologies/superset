...
set -uo pipefail
ws="$PWD"
[ -x "$ws/.superset/setup.cloud.sh" ] && "$ws/.superset/setup.cloud.sh"
[ -x "$ws/.superset/dev-stack.cloud.sh" ] && exec "$ws/.superset/dev-stack.cloud.sh"
echo "superset-dev-stack: this checkout has no .superset/dev-stack.cloud.sh"
DEVSTACK
