...
# agent drives) and a private one for automation that launches its own
# Chrome, since Chrome is single-instance per profile. Runs after
...
flags="$(sed -n 's/^exec \/usr\/bin\/google-chrome-stable \(.*\) "\$@"$/\1/p' /usr/local/bin/google-chrome)"
[ -n "$flags" ] || { echo "could not read the wrapper's flags" >&2; exit 1; }
desktop=/usr/share/applications/google-chrome.desktop
if [ -f "$desktop" ]; then
	sed -i -E "s|^Exec=/usr/bin/google-chrome-stable( .*)?( %U)?$|Exec=/usr/bin/google-chrome-stable ${flags} %U|" "$desktop"
fi
...
seed_profile "$home/.config/google-chrome"
