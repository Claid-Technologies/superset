...
Run `bash scripts/sweep.sh` from this skill's directory (`bash ${CLAUDE_SKILL_DIR}/scripts/sweep.sh` in Claude Code). It lists workspaces and tasks, then for each workspace lists its terminals and reads the last screen of every agent terminal, tolerating individual failures. It sweeps this machine by default; pass `--host <id>` to sweep a remote host. It never sweeps cloud workspaces, since reading each one wakes its sandbox. Pass `--max-lines <n>` to change the per-terminal tail (default 60).

If the script can't run (no bash, no `jq`), do the same by hand: `superset workspaces list --local --json`, `superset tasks list --json`, then per workspace `superset terminals list --local --workspace <id> --json` and `superset terminals read --local --workspace <id> --terminal <terminalId> --max-lines 60`.
